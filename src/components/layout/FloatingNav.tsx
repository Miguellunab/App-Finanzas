    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full px-4" style={{ maxWidth: '440px' }}>
      <div className="flex items-center justify-around rounded-3xl p-2" style={{ background: 'rgba(17,17,24,0.9)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
        {navItems.map((item) => {
          ...
          return (
            <a key={item.href} href={item.href} aria-label={item.label} className="relative p-3 rounded-2xl flex items-center justify-center no-underline" style={{ color: active ? item.color : '#5a5870', background: active ? `${item.color}20` : 'transparent', border: active ? `1px solid ${item.color}40` : '1px solid transparent' }}>
              <item.icon />
            </a>
          );
        })}
